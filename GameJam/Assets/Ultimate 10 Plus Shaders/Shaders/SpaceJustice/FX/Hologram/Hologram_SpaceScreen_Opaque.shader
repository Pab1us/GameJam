Shader "SpaceJustice/FX/Hologram/Hologram_SpaceScreen_Opaque"
{
	Properties
	{
		// General
		...
		_ScanTiling ("Scan Tiling", Range(0.01, 1.0)) = 0.05
	...
	SubShader
	{
		...
		Pass
		{
			...
			fixed4 frag (v2f i) : SV_Target
			{
				...
				fixed scan = abs(frac(dir * _ScanTiling * 0.25 + _Time.w * _ScanSpeed) *2.0 - 1.0);

				// Glow
				fixed 	glowPre = frac(dir * _GlowTiling * 0.01 + _Time.w * _GlowSpeed) - _GlowOffset;
