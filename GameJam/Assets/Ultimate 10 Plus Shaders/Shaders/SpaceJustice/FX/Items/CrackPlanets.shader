Shader "SpaceJustice/FX/Items/CrackPlanets"
{
	Properties
	{
		[Header(Curtain)]
	...
	SubShader
	{
		...
		Pass
		{
			...
			fixed4 frag (v2f i) : SV_Target
			{
				...
				fixed curtain =  saturate((i.uv1.y - _EvolutionFallOff - _Evolution)/ -_EvolutionFallOff);
                i.color.a *= curtain;
                //color
				fixed4 col;
				fixed tex_BW = tex2D(_Tex, i.uv.xy).r * curtain;
