Shader "SpaceJustice/FX/Scroll/Scroll2BW_Mask_Dissolve_Distort_Add"
{
	Properties
	{
	...
	SubShader
	{
		...
		Pass
		{
			...
			v2f vert (appdata v)
			{
				...
				o.uv1 = v.uv * _Tex1_TilingScroll.xy + frac(_Tex1_TilingScroll.zw * _Time.y);
			...
			fixed4 frag (v2f i) : SV_Target
			{
				...
				half2 dist = (distortion - 0.5f) * lerp(_Distort.zw, _Distort.xy, pow(i.color.a, _TexControl.w));
				...
				//color
				fixed4 col;
				half tex_BW = tex2D(_Tex, i.uv.xy).r * tex2D(_Tex1, i.uv1).r * _TexControl.x;
				fixed4 color1 = lerp(_Tex_ColorBlackTint, _Tex_ColorWhiteTint, pow(tex_BW, _TexControl.z));
				col.rgb = color1.rgb * i.color.rgb;
