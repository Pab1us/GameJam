Shader "SpaceJustice/Default (Legacy)"
{
	Properties
	{
		...
		[NoScaleOffset] _EmiTex ("Emission Texture", 2D) = "black" {}
		_EmiVal ("Intensity", Range(0., 5.)) = 0.
	...
	v2f vert(appdata v)
	{
		...
		// diffuse lighting
		fixed4 NdotL = saturate(dot(worldNormal, worldLightDir) * lightColor);
		...
		fixed RdotV = saturate(dot(refl, worldViewDir));
		fixed4 spec = pow(RdotV, _Shininess) * lightColor * saturate(NdotL) * _SpecColor;
		o.specular = spec;
		#endif
	...
	fixed4 frag(v2f i) : SV_Target
	{
		...
		#if _FOG_ON
		fixed4 fogColor = tex2D(_FogLUT, float2(i.fogCoord * _FogLUTParams.x + _FogLUTParams.y, 0.5));
