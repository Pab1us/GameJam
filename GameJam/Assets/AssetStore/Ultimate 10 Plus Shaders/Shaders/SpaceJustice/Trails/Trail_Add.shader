Shader "SpaceJustice/Trails/Trail Add"
{
	Properties
	{
	...
	SubShader
	{
		...
		#include "UnityCG.cginc"
			...
			v2f vert(appdata i)
			{
				...
				o.color *= _Color;
				o.color.rgb *= o.color.a;// * COLOR_MULTIPLIER * 2.0f;
				o.uv_alphaBlendFactor.z = saturate(max(max(o.color.r, o.color.g), o.color.b) * COLOR_MULTIPLIER - 1.0f); // 0 = alphablend, 1 = additive blend

				fixed alphaFix = saturate(min(max(o.color.a, 0.5f), 0.6f) * 3.0f);

				o.color.a *= alphaFix;//(1.0f - o.uv_alphaBlendFactor.z);
			#endif
			...
			fixed4 frag (v2f i) : SV_Target
			{
				...
				col.a *= 4.5f;//saturate(col.a + i.uv_alphaBlendFactor.z);
