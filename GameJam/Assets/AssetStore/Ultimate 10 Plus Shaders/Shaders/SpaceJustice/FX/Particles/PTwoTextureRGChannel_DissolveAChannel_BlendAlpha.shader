Shader "SpaceJustice/FX/Particle/PTwoTextureRGChannel_DissolveAChannel_BlendAlpha"
{
	Properties
	{
	...
	SubShader
	{
		...
		Pass
		{
			...
			fixed4 frag (v2f i) : SV_Target
			{
				...
				fixed tex_RG = tex_R * (1.0f - tex_G) + tex_G;
				fixed4 tex_RG_color = lerp(_Tex_ColorBlackTint, _Tex_ColorWhiteTint, pow(tex_RG, _Tex_ColorLinear));

				col.rgb = tex_RG_color.rgb * _Brighness * i.color.rgb;
