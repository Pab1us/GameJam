Shader "SpaceJustice/Background/Planet 2 Additional Light and Fog"
{
	Properties
	{
		...
		[Header(Fog)]
		[Toggle] _Fog("Enabled", Float) = 0.
		_FogColor ("Color", Color) = (0., 0., 0., 0.)
		_FogParams ("Start X, End Y", Vector) = (0., 0., 0., 0.)
	}
	...
	fixed4 frag(v2f i) : SV_Target
	{
		...
		fixed3 rim = smoothstep(_RimEnd, _RimEnd - _RimStart, saturate(dot(n, v_rim))) * _RimColor.a * _RimColor.rgb;
		color.rgb += rim * mask.r;

		/// fog
		float fogFactor = -wpos.z / (_FogParams.y - _FogParams.x) + _FogParams.y / (_FogParams.y - _FogParams.x);
		color.rgb = lerp(color.rgb, _FogColor.rgb, _Fog * saturate(1.0f - fogFactor));
